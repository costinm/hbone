...
func readMaxCPUFreq() {
	...
	if err != nil {
		...
		log.Printf("Could not read /proc/cpuinfo: %v", err)
		return
	}
	...
	for _, line := range strings.Split(cpuinfo, "\n") {
		if strings.Contains(line, "cpu MHz") {
			...
			if len(splitMHz) < 2 {
				log.Printf("Could not read /proc/cpuinfo: malformed cpu MHz line")
				return
			}
			...
			if err != nil {
				log.Printf("Could not parse cpu MHz value %v: %v", splitMHz[1], err)
				cpuFreqMHz = 0
				return
			}
			return
	...
	log.Printf("Could not parse /proc/cpuinfo, it is empty or does not contain cpu MHz")
