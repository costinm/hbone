...
// ReaderCopier is the main abstraction, representing a connection with metadata and additional
// helpers. The 'canonical' stream in a mesh is a H2 or H3 stream, represented as a http.Request
// and http.Response pair.
//
// In addition to the In/Out (corresponding to the body of req/res in h2 case) and headers, the ReaderCopier
...
type Stream struct {
	...
	// can be retrieved with ReaderCopier.ResponseWriter.
	// @Deprecated - use separate structure when using the h2/h3 stack.
	Request *http.Request `json:"-"`

	// Metadata to send. ReaderCopier implements http.ResponseWriter.
	...
	// net.Conn may be a ReaderCopier or a regular TCP/TLS connection.
...
//func (s *ReaderCopier) ConnectionState() tls.ConnectionState {
//	if s.TLS == nil {
...
func (s *Stream) SendHeader(w io.WriteCloser, h http.Header) error {
	...
	if DebugClose {
		log.Println("ReaderCopier.sendHeaders ", s.StreamId, h)
	}
	return nil
}
...
func (s *Stream) ReadHeader(in io.Reader) error {
	...
	if DebugClose {
		log.Println("ReaderCopier.receiveHeaders ", s.StreamId, s.InHeader)
	}
	return nil
}
...
func (s *Stream) RemoteAddr() net.Addr {
	...
	// non-test Streams are either backed by a net.ReaderCopier or a Request
	if cw, ok := s.Out.(net.Conn); ok {
